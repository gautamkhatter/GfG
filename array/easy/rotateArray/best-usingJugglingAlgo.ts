...
function rotateArrayUsingJuggling(
   nums: number[],
   leftRotateCount: number
): void {
   let arraySize = nums.length;
   leftRotateCount %= arraySize;

   // calculating number of cycles in the rotation
   let totalCycles = gcd(arraySize, leftRotateCount);

   // now we start processing each cycle;
   for (let cycleCount = 0; cycleCount < totalCycles; cycleCount++) {
      // start element of the current cycle
      let tempElement = nums[cycleCount];
      let currentPosition = cycleCount,
         nextPosition: number;

      while (true) {
         nextPosition = (currentPosition + leftRotateCount) % arraySize;
         if (nextPosition === cycleCount) break;
         nums[currentPosition] = nums[nextPosition];
         currentPosition = nextPosition;
      }

      nums[currentPosition] = tempElement;
   }
}
 ...
 * Function call:- rotateArrayUsingJuggling(nums);
 *
 * Function execution:
